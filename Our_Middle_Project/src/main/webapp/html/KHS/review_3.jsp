...
/* 기본: 아이콘 전환 애니메이션 */
#wrtBtn svg,
#iwrtBtn svg {              /* 버튼 id가 iwrtBtn이므로 이 셀렉터가 적용됩니다 */
  transition: transform .22s ease;
  transform-origin: 50% 50%;
}

/* 열림 상태(aria-expanded="true")에서 180도 회전 */
#iwrtBtn[aria-expanded="true"] svg {
  transform: rotate(180deg);
}

/* 접근성 배려: 모션 최소화 설정 시 애니메이션 비활성화 */
@media (prefers-reduced-motion: reduce) {
  #iwrtBtn svg { transition: none; }
}
...
.starBtn span {
  ...
  /* color는 여기서 지정하지 마세요!
     이미 .starsNew .starBtn span / .filled 에서 관리합니다. */
}
			...
			<aside id="iWrtReview" class="wrtReview">
				...
				<div class="wrtBody wrtBody--new" aria-live="polite">
					...
					<div class="wrtColLeftNew">
						<div class="rowNew">
							...
							<div id="starsGroup" class="starsNew" role="radiogroup"
								...
								<button class="starBtn" type="button" role="radio"
									aria-checked="false" data-val="1">
									<span>★</span>
								</button>
								<button class="starBtn" type="button" role="radio"
									aria-checked="false" data-val="2">
									<span>★</span>
								</button>
								<button class="starBtn" type="button" role="radio"
						...
						<div class="rowNew">
							<label class="wrtReviewLabel">이미지 첨부</label>
					...
					<div class="wrtColRightNew">
						...
						<div class="btnRowNew">
							<button id="btnSubmit" class="submitBtnNew" type="button" disabled>리뷰 등록</button>
						</div>
					</div>
				</div>
...
window.onload = () => {
  ...
  updateCounter(); validate(); setRating(0); renderPreview();
  ...
  btnSubmit.addEventListener('click', async ()=>{
    ...
    try{
      ...
      composer.close(); composer.toggleBtn.textContent='열기';
