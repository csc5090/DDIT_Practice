...
html, body {
	margin: 0;
	height: 100%;
	background-image: url('../../images/logins/loginBack.png');
}
...
/* =========================
   Review Page – Neon Green Edition (Full)
   ========================= */


/* 2) 리뷰 모달 다이얼로그 – 어두운 네온 초록 */
.reviewDialog {
  background: rgba(0, 0, 0, 0.25);
  border: 2px solid rgba(20, 200, 120, 0.8);
  box-shadow:
    0 0 20px rgba(20, 255, 160, 0.7),
    0 0 40px rgba(20, 255, 160, 0.5),
    0 0 80px rgba(20, 255, 160, 0.3),
    inset 0 0 20px rgba(20, 255, 160, 0.4);
  border-radius: 16px;
  color: #e8fff3;
}

/* 3) 헤더 – 반투명 어두운 배경 + 부드러운 네온 라인 */
.reviewHeader {
...
.reviewHeader .reviewTitle {
  color: white;
  text-shadow:
    0 0 8px rgba(16,185,129,.22),
    0 0 16px rgba(16,185,129,.18);
}

/* 4) 이너 컨텐츠 – 반투명 배경 + 은은한 그린 글로우 */
.innerContent {
  background: rgba(0, 0, 0, 0.35);
  backdrop-filter: blur(2px);
  -webkit-backdrop-filter: blur(2px);
  box-shadow: inset 0 0 18px rgba(16,185,129,.12);
  border-top: 1px solid rgba(16,185,129,.25);
  color: #e8fff3;
}

/* 5) 작성창 열기 버튼 (열림/닫힘 동일한 기본효과) */
#iwrtBtn {
  color: #a7f3d0;
  background: rgba(0, 0, 0, 0.25);
  ...
  box-shadow:
    ...
    inset 0 0 12px rgba(16, 185, 129, 0.25);
...
#iwrtBtn:hover {
  ...
  box-shadow:
    ...
    inset 0 0 16px rgba(16, 185, 129, 0.35);
}

/* 열림 상태 */
#iwrtBtn[aria-expanded="true"] {
  color: #34d399;
  background: rgba(0, 0, 0, 0.35);
  border-color: rgba(16, 185, 129, 0.85);
  box-shadow:
...
#btnSubmit:not([disabled]):hover {
  ...
  box-shadow:
    ...
    inset 0 0 16px rgba(16, 185, 129, 0.25);
}
...
.starsNew .starBtn.filled {
  border-color: #10b981;
  box-shadow:
    0 0 16px rgba(16,185,129,.24),
    inset 0 0 10px rgba(16,185,129,.10);
}
...
/* 10) 포커스 접근성 */
#iwrtBtn:focus-visible,
#iCloseModal:focus-visible,
...
<body>
  <div id="reviewRoot" class="reviewContainer">
...
window.onload = () => {
  ...
  window.addEventListener('click', (e) => {
    let root = e.target.closest('#reviewRoot');
    if (!root) return;

    // 모달 열기
    let openBtn = e.target.closest('#iOpenModal');
    if (openBtn) {
      let modal = root.querySelector('#iReviewModal');
    ...
    // 작성창 토글 (.wrtReview.open) — 아이콘은 aria-expanded만 토글! (텍스트/innerHTML 건드리지 않음)
    let wrtBtn = e.target.closest('#iwrtBtn');
    if (wrtBtn) {
      let wrt = root.querySelector('#iWrtReview');
      if (wrt) {
        let isOpen = wrt.classList.contains('open');
        wrt.classList.toggle('open', !isOpen);
        wrtBtn.setAttribute('aria-expanded', isOpen ? 'false' : 'true'); // CSS가 이 상태로 svg를 회전
      }
      return;
    }

    // 별점 선택 (#starsGroup .starBtn)
    ...
    if (star) {
      ...
      if (group) {
        ...
        // aria-checked (선택 하나) + .filled (선택값 이하에 시각 효과)
        ...
        let hidden = root.querySelector('#ratingVal');
    ...
    if (rm) {
      ...
      let input = root.querySelector('#imageInput');
      if (grid && input && input.files && window.DataTransfer) {
    ...
    if (submit) {
      let v = validate(root);
      if (!v.ok) {
        notify('확인 필요', v.msg, 'warning');
        return;
      }
      notify('검증 완료', '서버 연동은 구축 후 진행합니다.', 'success');
      resetForm(root);
      return;
    }
  });

  // 입력: 글자수 카운터 + 등록 버튼 활성화
  window.addEventListener('input', (e) => {
    let root = e.target.closest('#reviewRoot');
    if (!root) return;
    let ta = e.target.closest('#content');
    if (ta) {
      let cnt = root.querySelector('#cnt');
      if (cnt) cnt.textContent = (ta.value || '').length;
      updateSubmitState(root);
    }
  });

  // 파일 변경: 2장 제한 + 미리보기 + 카운터 + 버튼 활성화
  ...
  let renderPreview = (grid, files) => {
    ...
    files = files || [];
    if (!files.length) {
      arr.push('<div class="emptyBoxNew" data-empty>이미지 없음</div>');
    } else {
  ...
  let updateSubmitState = (root) => {
    ...
    let content = root.querySelector('#content');
    let textOk = content ? (content.value || '').trim().length >= 10 : false;
    btn.disabled = !(starOk && textOk);
  };
  ...
  let resetForm = (root) => {
    ...
    for (let i = 0; i < stars.length; i++) {
      stars[i].setAttribute('aria-checked','false');
  ...
  let notify = (title, text, icon) => {
    if (window.Swal && window.Swal.fire) window.Swal.fire({ title, text, icon });
