...
public class ReviewListController implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp)
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (i > 0)
				sb.append(',');
			sb.append('{').append("\"boardNo\":").append(r.getBoardNo()).append(',').append("\"memNo\":")
					.append(r.getMemNo()).append(',').append("\"nickName\":\"").append(esc(r.getNickName()))
					.append("\",").append("\"boardContent\":\"").append(esc(r.getBoardContent())).append("\",")
					.append("\"createdDate\":\"").append(esc(r.getCreatedDate())).append("\",")
					.append("\"updatedDate\":\"").append(esc(r.getUpdatedDate())).append("\",").append("\"star\":")
					.append(r.getStar())
